...
library(dplyr) #dplyr package lets us use group_by function
...
Senseandsensibility_sentiment<- tidy_data%>% filter(book=="Sense & Sensibility")%>%
inner_join(bing)%>%count(book = "Sense & Sensibility",index = linenumber %/% 80, sentiment)%>%
